...
const ProfileScreen = ({ navigation }) => {
    ...
    // Validación simple para el estado del botón
   ...
   const handleLogin = async () => {
    try {
        ...
        await loginRequest({ email, password }); // ✅ BIEN
    ...
    // --- VISTA: SI NO ESTÁ LOGUEADO (LOGIN FORM) ---
    return (
        <SafeAreaView style={styles.container}>
             <View style={styles.header}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
            ...
            <View style={[styles.content, { justifyContent: 'center' }]}>
                <View style={{ alignItems: 'center', marginBottom: 40 }}>
                    ...
                    <Text style={styles.welcomeSubtitle}>Inicia sesión para guardar tus favoritos</Text>
                </View>
                ...
                {/* BOTÓN ACTUALIZADO: Cambia de opacidad si no es válido */}
                <TouchableOpacity 
                style={[
                    styles.loginButton,
                    { opacity: isValid ? 1 : 0.6, backgroundColor: isValid ? COLORS.primary : '#999' }
                ]} 
                onPress={handleLogin}
                disabled={loading || !isValid}   // ✅ usamos loading del contexto
                >
                {loading ? (
                ...
                {/* BOTÓN ACTUALIZADO: Ahora tiene acción onPress */}
...
const styles = StyleSheet.create({
    ...
    logoutButton: {
        backgroundColor: '#FFF0F0', padding: 16, borderRadius: 12, alignItems: 'center'
    },
